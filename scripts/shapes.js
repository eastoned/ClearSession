...
export function drawRoad(start, end, sourceCount, childCount, pos, scene, roadMat, dataID, obj){
        ...
        if(childCount < 2){
            ...
            triGroup.name = "hello";
            drawTriangleSign((pos[start].y) - vec.x*15, (pos[start].x) - vec.y*15, 3, 5, 10, dotMaterial, triGroup, 1, angle + 90 * Math.PI/180, pos[end], scene);
            
            scene.add(triGroup);
            loadSign(end, dataID, obj, pos, scene, (pos[end].y) + vec.x*15, (pos[end].x) + vec.y*15);
            ...
            drawTriangleSign((pos[start].y) - vec.x*15, (pos[start].x) - vec.y*15, 3, 5, 10, dotMaterial, triGroup, 1, angle + 90 * Math.PI/180, pos[end]);
            scene.add(triGroup);
    ...
    export function drawTriangleSign(xPos, yPos, width, height, curveSize, mat, group, offset, rot, target, scene, interactive = true){
        ...
        signMesh.addEventListener('click', (event) => {
            moveCam(target.y, target.x, 0, 0, -50, 20);
            toggleScenePerspective(true, scene);
        });
   ...
   export function drawQuad(xPos, yPos, width, height, mat, group, offset){
        const shape = new THREE.Shape();
